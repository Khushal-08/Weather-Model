...
def plot_results(train_df, test_df, y_test_pred):
    ...
    plt.plot(test_df['date'], y_test_pred, label='Test Predictions (LightGBM)', color='red', alpha=0.8)
    
    plt.title('PM2.5 Forecasting: Actual vs Predicted (LightGBM)')
    plt.xlabel('Date')
    plt.ylabel('PM2.5')
    plt.legend()
    plt.grid(True, alpha=0.3)
    
    plt.savefig(REPORTS_DIR / "baseline_lightgbm_actual_vs_predicted.png")
    plt.close()
    ...
    plt.title('Residuals Over Time (Actual - Predicted) - LightGBM')
    plt.xlabel('Date')
    plt.ylabel('Error (PM2.5)')
    plt.grid(True, alpha=0.3)
    
    plt.savefig(REPORTS_DIR / "baseline_lightgbm_residuals.png")
    plt.close()
...
def main():
    ...
    # Print metrics to file for easy extraction
    pd.DataFrame([metrics]).to_csv(REPORTS_DIR / "lightgbm_metrics.csv", index=False)
    
    # 5. Plotting
    plot_results(train_df, test_df, y_test_pred)
    
    # 6. Save Model
    model_path = MODELS_DIR / "lightgbm_baseline.joblib"
